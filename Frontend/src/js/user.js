...
function buildUserMain() {
  ...
  main.innerHTML = `
    ...
    <div class="admin-panels user-panels">
      <section class="admin-panel user-logs-panel" aria-labelledby="user-logs-heading">
        <div class="admin-panel__head">
          <h2 class="admin-panel__title" id="user-logs-heading">Your Recent Logs</h2>
          <a class="admin-panel__action" href="ulogs.html">View all</a>
        </div>
...
function buildCurrentUserAliases(currentUser) {
  ...
  const aliases = new Set([
    ...
    normalizeToken(currentUser?.full_name),
...
function isLogRowForCurrentUser(row, aliases) {
...
function renderUserRecentLogs(main, rows) {
...
async function loadUserRecentLogs(main, currentUser) {
  const tbody = main.querySelector('#user-recent-tbody');
  if (!tbody) return;
  try {
    const res = await fetch(apiUrl('/api/activity-logs/'), { credentials: 'include' });
    if (!res.ok) {
      renderUserRecentLogs(main, []);
      return;
    }
    const payload = await res.json();
    const rows = Array.isArray(payload?.rows) ? payload.rows : [];
    const aliases = buildCurrentUserAliases(currentUser);
    const filtered = rows.filter((row) => isLogRowForCurrentUser(row, aliases)).slice(0, 5);
    renderUserRecentLogs(main, filtered);
  } catch {
    tbody.innerHTML = `
      <tr>
        <td colspan="4">Unable to load your recent logs.</td>
      </tr>
    `;
  }
}
...
async function mountUserPage(root = document.querySelector('#app')) {
  ...
  loadUserRecentLogs(main, currentUser);
