...
function buildIncomingMain(currentUser) {
  ...
  const canCompleteByDeptCache = new Map();
  ...
  const userCanMarkComplete = async () => {
    const deptKey = normalizeToken(currentUser?.office_department);
    if (!deptKey) return false;
    if (canCompleteByDeptCache.has(deptKey)) {
      return Boolean(canCompleteByDeptCache.get(deptKey));
    }
    try {
      const res = await fetch(apiUrl('/api/system-config/offices/'), { credentials: 'include' });
      if (!res.ok) {
        canCompleteByDeptCache.set(deptKey, false);
        return false;
      }
      const payload = await res.json().catch(() => ({}));
      const rows = Array.isArray(payload?.rows) ? payload.rows : [];
      rows.forEach((row) => {
        const key = normalizeToken(row?.name || row?.office_department || '');
        if (!key) return;
        canCompleteByDeptCache.set(key, isTruthyFlag(row?.can_mark_complete));
      });
      return Boolean(canCompleteByDeptCache.get(deptKey));
    } catch (error) {
      console.warn('Incoming: failed to load office completion permissions', error);
      canCompleteByDeptCache.set(deptKey, false);
      return false;
    }
  };
    ...
    try {
      ...
      if (canMarkComplete) {
        const completeRes = await fetch(apiUrl(`/api/outgoing-documents/${docId}/`), {
          ...
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ document_state: 'COMPLETED' }),
        });
