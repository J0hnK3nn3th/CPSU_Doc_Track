...
export function createSidebar({ activeId = 'dashboard', onSelect, isAdmin = true, dashboardHref = 'admin.html' } = {}) {
  ...
  const items = NAV_ITEMS
    ...
    .map((item) => {
      ...
      if (item.id === 'dashboard') return { ...item, href: 'user.html' };
      if (item.id === 'incoming') return { ...item, href: 'uincoming.html' };
      if (item.id === 'outgoing') return { ...item, href: 'uoutgoing.html' };
      if (item.id === 'reports') return { ...item, href: 'ureports.html' };
      if (item.id === 'logs') return { ...item, href: 'ulogs.html' };
      return item;
    });
